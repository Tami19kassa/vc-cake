...
export async function POST(request) {
  try {
    ...
    const duplicateReg = regs.find((r) => r.paymentReference.trim().toUpperCase() === savedRefCode);
    if (duplicateReg) {
    ...
    const duplicateOrder = orders.find((o) => o.paymentReference.trim().toUpperCase() === savedRefCode);
    if (duplicateOrder) {
      return NextResponse.json(
